...
from models import User, Donor, BloodRequest # Assuming Donor has get_by_blood_group_location
...
def search_donors():
    try:
        ...
        state = request.args.get('state') # New required parameter
        city = request.args.get('city')   # Now optional
        
        # Validate required parameters
        if not blood_group:
            return jsonify({'message': 'Blood group is a required search criterion.'}), 400
        if not state:
             # The frontend makes state required, so we should check for it here too
            return jsonify({'message': 'State/Region is a required search criterion.'}), 400

        # Pass all three parameters to a new, flexible database method
        # NOTE: You must update your 'models.py' and 'database.py' to implement 
        # Donor.get_by_blood_group_location(blood_group, state, city)
